...
class ShopCategory_model extends CI_Model {
    ...
    public function getShopCategoriesByParams($page, $perpage, $search_key, $sort_field, $sort_sort){
        $this->db->select("*");
    ...
    public function getShopCategoriesTotal($search_key){
        $this->db->select("*");
        $this->db->from($this->table);
        if($search_key!="")
            $this->db->like('category', $search_key);
