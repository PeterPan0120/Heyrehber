    ...
    init:function() {
        ...
        t=$("#m_my_guide_request_datatable").mDatatable( {
            data: {
                ...
                source: {
                    read: {
                        url: "/agency/guide_requests_getMyGuideRequests",
            ...
            columns:[ 
                ...
                }, 
                {
                    ...
                    width:100, 
                    template: function(t){
                        return "<a href='/agency/guide_requests_guide_request_detail?id="+t.id+"'>"+t.title+"</a>";
                    }
                },  
                {
                    field:"range", 
                ...
                }, 
                {
                    ...
                    template:function(t, e, a) {
                        ...
                        return  '\t\t\t\t\t\t'+
                                '<a href="/agency/guide_requests_guide_request_detail?id='+t.id+'" class="m-portlet__nav-link btn m-btn m-btn--hover-warning m-btn--icon m-btn--icon-only m-btn--pill" title="View Details">'+
                                    '\t\t\t\t\t\t\t'+
                                    '<i class="la la-eye"></i>'+
                                    '\t\t\t\t\t\t'+
                                '</a>'+
                                '\t\t\t\t\t\t'+
                                '<a href="/agency/guide_requests_edit_guide_request?id='+t.id+'" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Edit Details">'+
                                    '\t\t\t\t\t\t\t'+
...
jQuery(document).ready(function() {
    ...
    $("#m_guide_request_datatable").on("click", "a[title='Delete']", function(){
        ...
        swal({
            ...
            e.value
            ? window.location.href="/agency/guide_requests_delete_guide_request?id="+id
    ...
    $('.form-guide-requests-add-guide-request').submit(function(e){
        e.preventDefault();
        $.ajax({
            url: '/agency/guide_requests_save_guide_request',
            data: $(this).serialize(),
            type: 'post',
                ...
                if(res['status']=="success"){
                    swal(
                        ...
                        "success").then(function(){
                            window.location.href="/agency/guide_requests_my_guide_requests";
                        });
                }else{
                    swal(
                        "Error!",
    ...
    $('.form-guide-requests-edit-guide-request').submit(function(e){
        e.preventDefault();
        $.ajax({
            url: '/agency/guide_requests_update_guide_request',
            data: $(this).serialize(),
            type: 'post',
            success: function(res){
                if(res['status']=="success"){
