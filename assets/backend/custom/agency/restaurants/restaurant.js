    ...
    init:function() {
        ...
        t=$("#m_restaurant_datatable").mDatatable( {
            data: {
                ...
                source: {
                    read: {
                        url: "/agency/restaurants_getRestaurants",
            ...
            columns:[ 
                ...
                }, 
                {
                    ...
                    width:100, 
                    template: function(t){
                        return "<a href='/agency/restaurants_restaurant_details?id="+t.id+"'>"+t.name+"</a>";
                    }
                }, 
                {
                    field:"category", 
                ...
                }, 
                {
                    ...
                    template:function(t, e, a) {
                        return  '\t\t\t\t\t\t'+
                                '<a href="/agency/restaurants_restaurant_details?id='+t.id+'" class="m-portlet__nav-link btn m-btn m-btn--hover-warning m-btn--icon m-btn--icon-only m-btn--pill" title="View Details">'+
                                    '\t\t\t\t\t\t\t'+
                                    '<i class="la la-eye"></i>'+
                                    '\t\t\t\t\t\t'+
                                '</a>'+
...
jQuery(document).ready(function() {
    ...
    $("#m_restaurant_datatable").on("click", "a[title='Delete']", function(){
        ...
        swal({
            ...
            e.value
            ? window.location.href="/agency/restaurants_delete_restaurant?id="+id
    ...
    $('.form-restaurants-add-restaurant').submit(function(e){
        e.preventDefault();
        $.ajax({
            url: '/agency/restaurants_save_restaurant',
            data: $(this).serialize(),
            type: 'post',
                ...
                if(res['status']=="success"){
                    swal(
                        ...
                        "success").then(function(){
                            window.location.href="/agency/restaurants_restaurants";
                        });
                }else{
                    swal(
                        "Error!",
    ...
    $('.form-restaurants-edit-restaurant').submit(function(e){
        e.preventDefault();
        $.ajax({
            url: '/agency/restaurants_update_restaurant',
            data: $(this).serialize(),
            type: 'post',
            success: function(res){
                if(res['status']=="success"){
