    ...
    init:function() {
        ...
        t=$("#m_museum_suggestion_datatable").mDatatable( {
            data: {
                ...
                source: {
                    read: {
                        url: "/guide/museums_getMuseumSuggestions",
            ...
            columns:[ 
                ...
                },
                {
                    ...
                    width:100, 
                    template: function(t){
                        return "<a href='/guide/museums_museum_suggestion_detail?id="+t.id+"'>"+t.name+"</a>";
                    }
                }, 
                {
                ...
                },
                {
                    ...
                    template:function(t, e, a) {
                        ...
                        return  '\t\t\t\t\t\t'+
                                '<a href="/guide/museums_museum_suggestion_detail?id='+t.id+'" class="m-portlet__nav-link btn m-btn m-btn--hover-warning m-btn--icon m-btn--icon-only m-btn--pill" title="View Details">'+
                                    '\t\t\t\t\t\t\t'+
                                    '<i class="la la-eye"></i>'+
                                    '\t\t\t\t\t\t'+
                                '</a>'+
                                '\t\t\t\t\t\t'+
                                '<a href="/guide/museums_edit_museum_suggestion?id='+t.id+'" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Edit Details">'+
                                    '\t\t\t\t\t\t\t'+
...
jQuery(document).ready(function() {
    ...
    $("#m_museum_suggestion_datatable").on("click", "a[title='Delete']", function(){
        ...
        swal({
            ...
            e.value
            ? window.location.href="/guide/museums_delete_museum?id="+id
                ...
                if(res['status']=="success"){
                    swal(
                        ...
                        "success").then(function(){
                            window.location.href="/guide/museums_museum_suggestions";
                        });
                }else{
                    swal(
                        "Error!",
    ...
    $('.form-museums-edit-museum-suggestion').submit(function(e){
        ...
        $.ajax({
            url: '/guide/museums_update_museum',
            data: $(this).serialize(),
            type: 'post',
            success: function(res){
                if(res['status']=="success"){
