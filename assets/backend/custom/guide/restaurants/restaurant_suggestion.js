    ...
    init:function() {
        ...
        t=$("#m_restaurant_suggestion_datatable").mDatatable( {
            data: {
                ...
                source: {
                    read: {
                        url: "/guide/restaurants_getRestaurantSuggestions",
            ...
            columns:[ 
                ...
                }, 
                {
                    ...
                    width:100, 
                    template: function(t){
                        return "<a href='/guide/restaurants_restaurant_suggestion_detail?id="+t.id+"'>"+t.name+"</a>";
                    }
                }, 
                {
                    field:"category", 
                ...
                },
                {
                    ...
                    template:function(t, e, a) {
                        ...
                        return  '\t\t\t\t\t\t'+
                                '<a href="/guide/restaurants_restaurant_suggestion_detail?id='+t.id+'" class="m-portlet__nav-link btn m-btn m-btn--hover-warning m-btn--icon m-btn--icon-only m-btn--pill" title="View Details">'+
                                    '\t\t\t\t\t\t\t'+
                                    '<i class="la la-eye"></i>'+
                                    '\t\t\t\t\t\t'+
                                '</a>'+
                                '\t\t\t\t\t\t'+
                                '<a href="/guide/restaurants_edit_restaurant_suggestion?id='+t.id+'" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Edit Details">'+
                                    '\t\t\t\t\t\t\t'+
...
jQuery(document).ready(function() {
    ...
    $("#m_restaurant_suggestion_datatable").on("click", "a[title='Delete']", function(){
        ...
        swal({
            ...
            e.value
            ? window.location.href="/guide/restaurants_delete_restaurant_suggestion?id="+id
    ...
    $('.form-restaurants-add-restaurant').submit(function(e){
        e.preventDefault();
        $.ajax({
            url: '/guide/restaurants_save_restaurant_suggestion',
            data: $(this).serialize(),
            type: 'post',
                ...
                if(res['status']=="success"){
                    swal(
                        ...
                        "success").then(function(){
                            window.location.href="/guide/restaurants_restaurant_suggestions";
                        });
                }else{
                    swal(
                        "Error!",
    ...
    $('.form-restaurants-edit-restaurant').submit(function(e){
        e.preventDefault();
        $.ajax({
            url: '/guide/restaurants_update_restaurant_suggestion',
            data: $(this).serialize(),
            type: 'post',
            success: function(res){
                if(res['status']=="success"){
